...
get_server_ids() {
    ...
    server_response=$(curl -s --location ''$qwik_api'/servers/ids' \
    --header 'Authorization: Bearer '$access_token'')
    readarray -t server_ids < <(echo "$server_response" | jq -r '.id[]')
}

enable_cron_optimizer() {
    _note "Enabling cron optimizer for all apps under account"
    ...
    _success "Cron optimizer enabled for all eligible apps under account."
}
